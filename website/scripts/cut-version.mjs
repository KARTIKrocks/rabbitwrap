 ...
 *
 *   - versions are MAJOR.MINOR only ("0.16"), never patch ("0.16.0")
 ...
 * removal, a deprecation. Purely additive releases get an `_0.17+_` marker in
 * docs/ instead. See VERSIONING.md rules 1 and 2. This script cannot check
 * that condition, so it is on you.
 *
 * Usage: npm run cut-version -- 0.17
 */
...
if (!/^\d+\.\d+$/.test(version)) {
  fail(
    `"${version}" is not a MAJOR.MINOR version.\n\n` +
      '  Snapshots are cut per minor release only — "0.17", not "0.17.0" or "v0.17".\n' +
